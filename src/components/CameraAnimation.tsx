...
const CameraAnimation: React.FC<CameraAnimationProps> = ({ children }) => {
  ...
  useEffect(() => {
    const sequence = async () => {
      ...
      setShowFlash(false);
      setZoomInLens(true);
      await new Promise(resolve => setTimeout(resolve, 1000)); // Delay to show zoom effect
      setShowContent(true);
    };
    sequence();
  }, [controls]);
  ...
  if (showContent) {
    return (
      <motion.div className="relative flex justify-center items-center h-screen bg-black"
      initial={{opacity: 0}}
    ...
    <div className="relative flex justify-center items-center h-screen bg-black overflow-hidden">
      ...
      {showFlash && (
        <motion.div
          ...
          animate={{ opacity: [1, 0.8, 0.6, 0.4, 0],scale:1 }}
          transition={{ duration: 0.1, repeat:1, repeatType:"reverse",opacity:[0,0.4,0.6,0.8,1] }}
        />
      )}
      <motion.div
        className={`${zoomInLens ? 'absolute -inset-36 z-20 transform' : 'relative'} flex justify-start`}
        initial={{ opacity: 0, scale: 0.2 }}
        animate={zoomInLens ? { opacity: 0.85, scale: 4,x:-1000,y:330 } : controls}
        transition={{ duration: 1 , opacity: 0.3}}
      >
        <Image src={camera} alt="Camera" layout={zoomInLens ? "fill" : "intrinsic"} objectFit={zoomInLens ? "cover" : "contain"} className={`${zoomInLens ? '-inset-44':''}`} loading='eager'/>
        <div className={`absolute inset-0 bg-black ${zoomInLens ? 'bg-opacity-65':'bg-opacity-80 '}`}></div>
      </motion.div>
    </div>
  );
};
